...
class GenerateCommandRunner extends CommandRunner<int> {
  GenerateCommandRunner() : super('generate', 'generates the bindings via ffi gen') {
    Directory(path.join(Directory.current.path, 'bin', 'bindings'))
        .listSync()
        .whereType<File>()
...
class GenerateBidingsCommand extends Command<int> {
  ...
  Future<int> run() async {
    final process = await Process.start(
      'dart',
      'run ffigen --config bin/bindings/$name.yaml'.split(' '),
      runInShell: true,
